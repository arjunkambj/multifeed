          ...
          {trend == null ? (
            ...
            <div className="flex items-center justify-between gap-2 text-xs">
              <span className={`flex items-center gap-1 font-medium ${trendColor}`}>
                <Icon
                  icon={trend < 0 ? "hugeicons:arrow-down-02" : "hugeicons:arrow-up-02"}
                  width={13}
                />
                {trend > 0 ? "+" : ""}{trend}%
              </span>
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {metrics === undefined ? (
        ...
        <section aria-label="Publishing KPIs" className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
          <MetricCard
            title="Scheduled posts"
            value={metrics.scheduledPosts.toLocaleString()}
            icon="hugeicons:calendar-03"
            change={percentageChange(metrics.scheduledPosts, metrics.previousScheduledPosts)}
          />
          <MetricCard
            title="Published posts"
            value={metrics.publishedPosts.toLocaleString()}
            icon="hugeicons:sent"
            change={percentageChange(metrics.publishedPosts, metrics.previousPublishedPosts)}
          />
          <MetricCard
            title="Publishing success"
            value={`${metrics.publishingSuccessRate.toFixed(1)}%`}
            icon="hugeicons:checkmark-badge-01"
            change={metrics.publishingSuccessRate - metrics.previousPublishingSuccessRate}
          />
          <MetricCard
            title="Engagements"
            value={metrics.engagement.toLocaleString()}
            icon="hugeicons:favourite"
            change={percentageChange(metrics.engagement, metrics.previousEngagement)}
          />
          <MetricCard
